  ...
  return (
    <footer className="bg-gray-900 text-gray-300 mt-16">
      <div className="max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          <img src={settings.logo || "/images/macluck-logo.svg"} alt={settings.siteName || "MacLuck"} className="h-16 w-auto mb-3 brightness-0 invert" />
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            ...
            <li><a href={`mailto:${settings.email || ''}`}>{settings.email || '[УКАЖИТЕ EMAIL]'}</a></li>
            <li>{settings.hours || '[УКАЖИТЕ ВРЕМЯ РАБОТЫ]'}</li>
          </ul>
          <div className="flex flex-wrap gap-3 mt-4 text-sm">
            {Object.entries(settings.social || {}).filter(([, url]) => url).map(([name, url]) => <a key={name} href={url} target="_blank" rel="noreferrer" className="hover:text-white">{name}</a>)}
          </div>
        </div>
